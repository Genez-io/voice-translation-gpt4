...
async def translate_audio(request: Request):
    ...
    if not audio_base64:
        print("No audio data provided or invalid audio format")
        return {"error": "No audio data provided or invalid audio format"}, 400
    ...
    if not response_json:
        print("Failed to process audio with GPT-4")
        return {"error": "Failed to process audio with GPT-4"}, 500
    ...
    print(f"Success, {source_transcript}, {target_transcript}, {retranslated_text}")
    return {
